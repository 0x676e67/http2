...
async fn handle_write_contention_request(
    mut request: Request<http2::RecvStream>,
...
async fn send_chunk(
    ...
    loop {
        ...
        match (Capacity { send }).await {
            ...
            None => return Err(http2::Reason::INTERNAL_ERROR.into()),
        }
    }
}

struct Capacity<'a> {
    send: &'a mut http2::SendStream<Bytes>,
}

impl Future for Capacity<'_> {
    type Output = Option<Result<usize, http2::Error>>;
